...
- Limits: Pi has no declarative hook JSON contract, timeout, failure, or process-output policy. Those semantics belong to the embedded Agent Bundler runtime. The generated descriptor is `hooks/hooks.v1.json`, not a Pi-native manifest.
- Validation: Pi has no stable offline non-mutating package validator. `pi install`, package load, and extension smoke tests mutate settings or execute extension code, so they are test-only, opt-in, and isolated with temporary configuration roots.
...
- Portable events: `sessionStart`, `sessionEnd`, `userPromptSubmitted`, `preToolUse`, `postToolUse`, `postToolUseFailure`, `agentStop` (portable stop), `notification`, and `preCompact`. There is no `postCompact` event in the documented list.
- Match and decisions: `preToolUse` can allow, deny, ask, or replace arguments; portable block and rewrite-input map there. Matchers are anchored regexes on supported event values. Hook entries of one event execute in order. Notification is inherently asynchronous and never blocks.
...
- Validation: no stable official offline non-mutating plugin validator is documented. Install/list/load smoke tests are test-only, opt-in, and isolate `COPILOT_HOME` and cache paths.
...
- Match and decisions: `preToolUse` supports `permission: allow|deny` and `updated_input`; `beforeSubmitPrompt` can continue or block. Event-specific pre-execution hooks can allow/deny, and documented `failClosed: true` exists only for applicable blocking events. Session lifecycle and compaction hooks are observational/fire-and-forget where documented.
...
- Validation: Cursor documents no stable official offline non-mutating plugin validator. Local `--plugin-dir` and load/fire smoke tests are test-only, opt-in, and use isolated temporary user/config roots.
