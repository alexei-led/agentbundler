...
- Match and decisions: tool-event matchers can select native tool names. Claude's `PreToolUse` can explicitly deny a tool call and can return updated input, but those native results use Claude's process protocol. Until Agent Bundler generates a translator from one canonical decision protocol, portable block and rewrite capabilities are rejected. Async command hooks are usable only where the portable hook is passive.
...
- Package paths: `${PLUGIN_ROOT}` refers to plugin files. Hook command entries are shell commands (`bash`, `powershell`, or cross-platform `command`). Agent Bundler emits separate Bash and PowerShell fields for portable exec handlers, with shell-specific quoting for literal and package-file arguments; explicit shell handlers retain the author-supplied cross-platform `command`.
...
- Match and decisions: Copilot `PreToolUse` can allow, deny, ask, or replace arguments, but Agent Bundler rejects portable block and rewrite capabilities until a target-owned process-protocol translator exists. PascalCase `PreToolUse` uses the documented Claude-compatible matcher names. Hook entries of one event execute in order. Notification is inherently asynchronous and never blocks.
...
- Match and decisions: matchers are regular expressions over mapped tool names. Grok `PreToolUse` is the only blocking event and supports explicit deny, but Agent Bundler rejects portable block decisions until a target-owned process-protocol translator exists. Grok documents no input-rewrite decision.
