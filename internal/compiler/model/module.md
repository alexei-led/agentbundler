...
- `exec` requires a non-empty program and forbids `shellCommand`; `shell` requires `shellCommand` and forbids program/arguments. A hook argument has exactly one of `literal` or `packageFile`.
...
- Timeout is positive and bounded. Asynchronous execution is allowed only for passive events; it cannot carry blocking, rewrite-input, or closed-failure semantics.
