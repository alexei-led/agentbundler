...
func TestRenderDeterministicBaseline(t *testing.T) {
	...
	if first.Target != model.TargetClaude {
		t.Fatalf("Render() target = %q, want %q", first.Target, model.TargetClaude)
	}
	...
	wantPaths := []model.RelativePath{
		"package-index.json",
		"packages/alpha/assets/agent/reviewer/asset.json",
		"packages/alpha/assets/agent/reviewer/content.md",
	...
	if got, want := string(plannedFile(t, first, "package-index.json").Bytes), "{\"format\":\"agentbundler-target-bundle\",\"formatRevision\":1,\"packages\":[\"alpha\",\"zeta\"],\"target\":\"claude\"}\n"; got != want {
		t.Fatalf("package-index.json = %q, want %q", got, want)
	}
	if got, want := string(plannedFile(t, first, "packages/alpha/package.json").Bytes), "{\"identity\":\"alpha\",\"metadata\":{\"labels\":[\"x\",\"y\"],\"name\":\"Alpha\"},\"target\":\"claude\"}\n"; got != want {
