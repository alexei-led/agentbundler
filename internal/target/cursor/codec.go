...
func catalogManifest(catalog marketplace.Catalog) (packageoutput.CatalogManifest, error) {
	...
	for _, entry := range catalog.Entries {
		document.Plugins = append(document.Plugins, catalogEntry{
			Name: entry.Name, Source: cursorMarketplaceSource(entry.Source), Description: entry.Description,
	...
	return packageoutput.CatalogManifest{Path: ".cursor-plugin/marketplace.json", Bytes: append(data, '\n')}, nil
}

func cursorMarketplaceSource(root string) string {
	if root == "." {
		return "./"
	}
	return "./" + root
