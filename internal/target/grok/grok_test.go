...
func TestRenderDeterministicBaseline(t *testing.T) {
	...
	if first.Target != model.TargetGrok {
		t.Fatalf("Render() target = %q, want %q", first.Target, model.TargetGrok)
	}
	...
	if got, want := string(plannedFile(t, first, "package-index.json").Bytes), "{\"format\":\"agentbundler-target-bundle\",\"formatRevision\":1,\"packages\":[\"alpha\",\"zeta\"],\"target\":\"grok\"}\n"; got != want {
		t.Fatalf("package-index.json = %q, want %q", got, want)
	}
	if got, want := string(plannedFile(t, first, "packages/alpha/package.json").Bytes), "{\"identity\":\"alpha\",\"metadata\":{\"labels\":[\"x\",\"y\"],\"name\":\"Alpha\"},\"target\":\"grok\"}\n"; got != want {
