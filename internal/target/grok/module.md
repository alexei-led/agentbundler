...
Documented portable events are session start/end, prompt submit, pre/post tool, post-tool failure, stop, notification, pre-compact, and post-compact. Matchers are regexes over mapped tool names. Grok `PreToolUse` supports explicit deny, and Agent Bundler uses a target-owned process-protocol translator for portable block decisions. Grok documents no input-rewrite decision. `hook.decision.rewrite-input` remains unsupported. `hook.failure.closed` is advisory for pre-tool hooks through the translator and requires an exact acknowledgment; other events reject it. Grok does not document asynchronous command handlers, so `hook.async` is unsupported. Claude-compatible agents render as Markdown, but Claude-only `sandbox_mode` is rejected.
