...
The runtime implements the portable exec/shell process protocol, package-file resolution, matchers, deterministic hook order, bounded output, timeout, cancellation, and failure-policy translation. Fail-closed is accepted only for pre-tool hooks, where Pi's `tool_call` block result can stop execution. Other Pi extension errors are logged while execution continues, so every other event requires fail-open. Hook subprocesses inherit only `PATH`; on Windows they also inherit `PATHEXT`, `SYSTEMROOT`, `WINDIR`, and `COMSPEC` (matched case-insensitively). All other ambient variables, including credentials, are omitted. Pi session shutdown cancels earlier work, then waits for session-end hooks to complete or reach their configured timeout. Explicit runtime shutdown and Pi cancellation signals abort session-end work. Unsupported event or decision cells fail before output.
